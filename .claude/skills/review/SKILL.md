...
Linear flow: **Needs-Review → In-Process → Finished** (clean) **or Needs-Revision**
...
3. Claim each in **one** `save_issue` write — the **claim** from `docs/agents/issue-tracker.md` and
   `state: "In-Process"` — one at a time, checking each answer; drop any lost race
   and take the next.
...
Confirm on Linear that each issue left In-Process and is unassigned, with a
